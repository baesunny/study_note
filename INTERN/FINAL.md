...
### 📍 01. 대규모 raw 데이터를 직접 정제하여 원하는 인사이트를 도출해냈다.
  ...
  그래서 이 분포를 기준으로 무의미한 데이터를 삭제하였으며 문제를 훨씬 간단하게 만들 수 있었다.  
...
### 📍 01. 프로젝트 별 마감기한을 보고할 때, 내가 예상하는 기간보다 플러스 알파로 기한을 설정하는 것이 좋다. 
  
  예를 들면 3일 내에 가능할 것 같아도 5일 걸릴 것 같다고 말씀드릴 것.
  
  데이터 분석 업무의 경우, 내가 생각하지 못한 변수가 발생할 수 있기에 
  이를 수습할 시간과 피드백을 반영할 시간을 고려하여 마감가능한 기한을 보고하는 것이 좋다.

  정말 급한상황이 아닌 이상, 기한보다 늦게 끝내는 것보다는 일찍 끝내는 것이 더 좋을테니 말이다.

∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙∙

### 📍 02. 애매한 상황에서 내 짐작대로 하기보단 조금 눈치보여도 확인하고 하는 것이 이억오천배정도 더 낫다.

  업무를 하다보면 방향성이 애매해지거나 사소한 것에 혼란이 생길 때가 있다.
  이럴 때마다 원래는 일단 내 짐작대로 한 뒤에 '이런 식으로 해봤습니다~'하고 보고 드렸는데 결국 다시 해야하는 상황이 발생했었다.

  잘하고 싶은 마음에 최대한 질문하지 않고 해결해보려는 마음이었지만 오히려 일을 두 번 해야하는 비효율적인 상황이 생긴 것이다.
  그래서 그 다음부터는 '일단 질문해보자'는 마인드를 갖게 되었다.

  그리고 이러한 마인드는 업무를 훨씬 완성도있게 할 수 있게 해주었다.
  '현재까지 진행한 것에 대한 검토 + 앞으로의 방향성에 대한 조언'을 들을 수 있으니 잘 끝낼 수밖에 없었던 것 같다.

  질문하는 것은 부끄러운 것이 아니다. 
  앞으로도 애매할 땐 무조건 질문하고 확실하게 확인한 뒤에 진행하는 연습을 해야겠다. 
...
음 이건 개인적인 내용도 많을 것 같아서 개인 에세이 블로그에 남기려고 한다.
